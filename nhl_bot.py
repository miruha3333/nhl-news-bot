...
# Рабочий RSS-поток через RSS.app
...
def clean_raw_rss_text(raw_text):
    if not raw_text:
        return ""
    # Удаляем HTML-теги, которые RSS.app добавляет в описание
    text = re.sub(r"<[^>]+>", "", raw_text)
    # Удаляем служебные приписки RSS-генераторов
...
def main():
    ...
    for entry in reversed(feed.entries[:10]):
        # Для уникальности и истории предпочтительнее id/link или очищенный заголовок
